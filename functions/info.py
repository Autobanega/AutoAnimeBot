    ...
    async def rename(self, original=False):
        try:
            ...
            if anime_name and self.data.get("episode_number"):
                return (
                    f"[ [ROW] S{self.data.get('anime_season') or 1}-{self.data.get('episode_number') or ''}] {(await self.get_english())} [{self.data.get('video_resolution')}] @TheRealmOfWeebs.mkv".replace(
                        "‘", ""
                    )
                    .replace("’", "")
                    .strip()
